...
以上内容均可在 App 中修改。固定地点默认只是上海示例坐标，安装后在设置中点一次
“使用当前位置”即可自动填写自己的天气地点、坐标和时区。
...
2. App 已预填当前服务地址；在“地点与规则设置”中填写令牌，然后点“使用当前位置”
   并允许使用 App 时定位。App 只获取一次位置，不会持续跟踪。
...
- 天气地点是一次定位后保存的固定坐标；不含地址搜索、持续定位或后台定位，
  手动经纬度保留为高级兜底。
